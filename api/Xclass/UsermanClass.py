...
from api.models import assets,agroups,users
from api.lib import XcloudLib
from uuid import uuid1
class UsermanHandle(object):
    ...
    def _addPerson(self):
        assets_u = users.objects.filter(UserName=self.parame['username'],).first()
        if assets_u:
            self.return_data['code'] = 10000
            self.return_data['msg'] = "您输入的联系人信息已经存在."
        else:
            print self.parame
            #{u'username': u'shengyuan', u'status': u'1', u'name': u'\u76db\u539f', u'power': u'2', u'action': u'addPerson', u'email': u'33'}
            users(uuid=uuid1(),
                  UserName = self.parame['username'],
                  Name = self.parame['name'],
                  Password = '',
                  Groups = '',
                  Email = self.parame['email'],
                  Mobile = '',
                  Status = self.parame['status'],
                  Power= self.parame['power'],
                  HeadPortrait = '',
                  HostNum = 0,
                   ).save()
            '''
