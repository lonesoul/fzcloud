...
class HandleAccess:
	...
	def CreateAccess(self):
		...
		c_chars = random.sample(init_chars, 4)
		KeyRandomId = ''.join(c_chars)
		KeyID = hashlib.md5(self.Data['UserId']+KeyRandomId).hexdigest()
		KeySecret = hashlib.sha256(KeyID).hexdigest()
		inaccess = AccessInfo(KeyID=KeyID,KeySecret=KeySecret,AccessStatus=0,KeyUseType='',KeyRandomId=KeyRandomId,UserId=self.Data['UserId'])
		inaccess.save()
		return_data = {
			"code" : 0,
			"msg" : '创建成功',
